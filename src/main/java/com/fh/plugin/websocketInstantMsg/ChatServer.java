...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("FHadminqq313596790")){
			String user = message.replaceFirst("FHadminqq313596790", "");
			WebSocket webSocket = ChatServerPool.getWebSocketByUser(user);
			if(webSocket!=null) {
				this.userLeave(webSocket);
			}
		...
		}/*if(null != message && message.contains("ping")) {
			ChatServerPool.sendMessageToUser(conn, "pong");//同时向本人发送消息		
		}*/else{
	...
	public void userjoin(String user, WebSocket conn){
		...
		//获取但那个钱所有在线用户，并进行一定的排序
		List<String> list = (List<String>)ChatServerPool.getOnlineUser();
		Collections.sort(list);
		result.element("list", list);
