...
//						}
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				allmenuList = this.getAttributeMenu(user.getRole().getROLE_ID(),session, USERNAME, roleRights, getArrayRoleRights(ROLE_IDS));			//菜单缓存
	...
	public ModelAndView homeIcon()throws Exception{
		...
		aaa(allmenuList);//allmenuList删除为false的
	...
	public JSONObject homeIconLoad()throws Exception{
		...
		list=aaa3(list,menuHasSelected,allmenuList);
		JSONObject jsonObject=new JSONObject();
		jsonObject.put("list", list);
		return jsonObject;
	}

	public static List<Object> aaa3(List<Object> list, List<PageData> menuHasSelected, List<Menu> allmenuList) {
		for (int i = 0; i < menuHasSelected.size(); i++) {
			for (int j = 0; j < allmenuList.size(); j++) {
				if (menuHasSelected.get(i).getString("MENU_ID").equals(allmenuList.get(j).getMENU_ID())) {
					List<Object> list2 = new ArrayList<>();
					List<Object> list3 = new ArrayList<>();
					list3.add(allmenuList.get(j).getMENU_ID());
					list3.add(allmenuList.get(j).getMENU_NAME());
					list3.add(allmenuList.get(j).getMENU_URL());
					list2.add(list3);
					if (allmenuList.get(j).getSubMenu().size() != 0) {
						aaa3(list2, menuHasSelected, allmenuList.get(j).getSubMenu());
					}
					list.add(list2);
				}
			}
	...
	public static void aaa(List<Menu> allmenuList){
		Iterator<Menu> iterator = allmenuList.iterator();
		while(iterator.hasNext()) {
			Menu menu=iterator.next();
			if(menu.isHasMenu()==true) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Menu> getAttributeMenu(String ROLE_ID,Session session, String USERNAME, String roleRights, List<String> arrayRoleRights) throws Exception{
		List<Menu> allmenuList = new ArrayList<Menu>();
		if(null == session.getAttribute(USERNAME + Const.SESSION_allmenuList)){	
			allmenuList = menuService.listAllMenuQx("0");							//获取所有菜单
			if(Tools.notEmpty(roleRights) || (ROLE_ID != null && "1".equals(ROLE_ID))){
				allmenuList = this.readMenu(ROLE_ID,allmenuList, roleRights, arrayRoleRights);				//根据角色权限获取本权限的菜单列表
			}
	 ...
	 * @return
	 */
	public List<Menu> readMenu(String ROLE_ID,List<Menu> menuList,String roleRights, List<String> arrayRoleRights){
		for(int i=0;i<menuList.size();i++){
			Boolean b1 = RightsHelper.testRights(roleRights, menuList.get(i).getMENU_ID());
			menuList.get(i).setHasMenu(b1); //赋予主职角色菜单权限
			if(ROLE_ID!= null && "1".equals(ROLE_ID)) {
				menuList.get(i).setHasMenu(true); //赋予主职角色菜单权限
			}else {
				menuList.get(i).setHasMenu(b1); //赋予主职角色菜单权限
			...
			if(menuList.get(i).isHasMenu()){		//判断是否有此菜单权限
				this.readMenu(ROLE_ID,menuList.get(i).getSubMenu(), roleRights, arrayRoleRights);//是：继续排查其子菜单
			}
		}
		return menuList;
	}
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value="/login_defaultFrame")
	public ModelAndView defaultFramePage() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		mv.addObject("pd",pd);
		mv.setViewName("system/index/defaultFrame");
		return mv;
	}
