...
function back(){
	...
	window.location.href = "xunjian.html?userName="+userName+"&userID="+userID+"&workshop="+workshop+"&equname="+equname+"&equCode="+equCode+"";
}
/*退出按钮*/
function exit(){
			...
			$.each(dataArr,function(index, item){
				...
				if(type == "数值"){
					for(var i=0;i<num+1;i++){					
						append0 = append0 + "<input id='num"+index+''+i+"' onchange='submitNumberData("+(i+1)+",this.value,this.id)'  value='"+resArr[i]+"' class='textInput' style='margin-right: 5px;width:10%;border-top:none;border-left:none;border-right:none;border-bottom:1px solid #000' type='text' disabled>";
					}					
				}else if(type == "开关"){
					for(var j=0;j<num+1;j++){
						if(resArr[j] == "1"){
							append0 = append0 + "<input id='check"+index+''+j+"' checked onclick='submitCheckData("+(j+1)+",this.id)' value='' class='checkInput' style='margin-left: 5px;border:1px solid #000' type='checkbox' disabled><span style=''>第"+(j+1)+"次</span>";
						}
						else{
							append0 = append0 + "<input id='check"+index+''+j+"' onclick='submitCheckData("+(j+1)+",this.id)' value='' class='checkInput' style='margin-left: 5px;border:1px solid #000' type='checkbox' disabled><span style=''>第"+(j+1)+"次</span>";
						}						
					}						
				}
