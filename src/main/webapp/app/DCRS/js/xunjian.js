...
function getXunJianRecords(a){
	...
	$(".main_content").html("");
	$.ajax({
		url:locationUrl + '/dafeng_mes/inspectionrecord1/listByCode?INSPECTIONTICKETCATEGORY=04&EQUIPMENTCODE='+equCode,
		type:'post',
		cache:false,
		success: function(data){
			console.log(data);
...
function jumpToDetail(id){	
	...
	var equCode = $("#equCode").val();
	window.location.href = "xunjianxiangqing.html?userName="+userName+"&userID="+userID+"&id="+id+"&workshop="+workshop+"&equname="+equname+"&equCode="+equCode+"";
}
/*获取url参数 */
