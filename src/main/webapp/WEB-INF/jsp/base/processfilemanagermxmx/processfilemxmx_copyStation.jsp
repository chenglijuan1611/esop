			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="processfilemanagermxmx/${msg}.do" method="post" name="Form" id="Form">
						<input type="hidden" name="PROCESSFILEMANAGERMX_ID" id="PROCESSFILEMANAGERMX_ID" value="${pd.PROCESSFILEMANAGERMX_ID}"/>
						<input type="hidden" name="PRODUCTLINE_NAME" id="PRODUCTLINE_NAME" value="${pd.PRODUCTLINE_NAME}"/>
						<input type="hidden" name="MATERIAL_CODING" id="MATERIAL_CODING" value="${pd.MATERIAL_CODING}"/>
						<input type="hidden" name="materialMxId" id="materialMxId" value="${pd.materialMxId}" />
...
<!-- 									</th> -->
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" title="复制" onclick="copy('${var.materialMxId}','${var.PRODUCT_CODE}');">
															复制
														</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
		...
		//检索
		function tosearch(){
			document.getElementById("Form").action = "processfilemanagermxmx/search2.do";
			top.jzts();
			$("#Form").submit();
		}
		$(function() {
// 			$("#PROCESS_CODING").attr("value",pd.PROCESS_CODING);
		...
		//复制物料的配置信息
		function copy(materialMxId,copyMaterial){
			bootbox.confirm("确定要复制吗?", function(result) {
 				top.jzts();
				if(result) {
					document.getElementById("Form").action = "processfilemanagermxmx/copy.do?MATERIAL_COPY2="+materialMxId+"&copyMaterial="+copyMaterial;
                	$("#Form").submit();
				}
			});
		}
