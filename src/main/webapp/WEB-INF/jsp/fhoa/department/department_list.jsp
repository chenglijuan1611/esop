					...
					<div class="row">
						<div class="col-xs-12">
							...
							<form action="department/list.do" method="post" name="Form"
										...
										<td>&nbsp; <select name="DEPARTMENT_ID"
											id="DEPARTMENT_ID">
												<option value="${DEPARTMENT_ID}"
													<c:if test="${DEPARTMENT_ID != ''}">selected</c:if>>本级</option>
												<option value=""
													<c:if test="${DEPARTMENT_ID == ''}">selected</c:if>>全部</option>
										</select>
										</td>
										<c:if test="${QX.cha == 1 }">
								...
								<table id="simple-table" class="table table-striped table-bordered table-hover" style="height:100%; overflow:scroll;margin-top: 5px;">
									<thead>
										<tr>
...
<!-- 															<td class='center'><a -->
<%-- 																href="javascript:goSondict('${var.DEPARTMENT_ID }')"><i --%>
<%-- 																	class="ace-icon fa fa-share bigger-100"></i>&nbsp;${var.NAME}</a></td> --%>
															<td class='center'><a
																href="javascript:goSondict('${var.DEPARTMENT_ID }')">&nbsp;${var.NAME}</a></td>
															<%-- <td class='center'><a
																href="javascript:goSondict('${var.DEPARTMENT_ID }')">${var.NAME_EN}</a></td> --%>
																...
																<div class="hidden-sm hidden-xs btn-group">
																	...
																	<c:if test="${QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"
																			onclick="del('${var.DEPARTMENT_ID}');"> <i
										...
										<tr>
											<td style="vertical-align: top;"><c:if
													...
													<a class="btn btn-sm btn-success"
														onclick="add('${DEPARTMENT_ID}');">新增</a>
												</c:if> <c:if
