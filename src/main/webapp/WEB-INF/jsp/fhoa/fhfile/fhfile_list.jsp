												...
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															...
															<td class='center'><img style="margin-top: -3px;"
																alt="${var.NAME}"
																...
																&nbsp; <c:if test="${var.fileType == 'tupian' }">
																	<a style="cursor: pointer;"
																		onmouseover="showTU('uploadFiles/uploadFile/${var.FILEPATH}','yulantu${vs.index+1}');"
																		onmouseout="hideTU('yulantu${vs.index+1}');">[预览]</a>
																</c:if> <c:if test="${var.fileType == 'pdf' }">
																	<a style="cursor: pointer;"
																		onclick="goViewPdf('${var.NAME}${fn:substring(var.FILEPATH ,19,fn:length(var.FILEPATH))}','${var.FHFILE_ID}');">[预览]</a>
																</c:if> <c:if test="${var.fileType == 'wenben' }">
																	<a style="cursor: pointer;"
																		onclick="goViewTxt('${var.NAME}${fn:substring(var.FILEPATH ,19,fn:length(var.FILEPATH))}','${var.FHFILE_ID}','gbk');">[预览]</a>
																</c:if>
																<div class="yulantu" id="yulantu${vs.index+1}"></div></td>
															<td class='center'>${var.USERNAME}-${var.zindex }</td>
															...
															<td class="center" style="width: 150px;"><c:if
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<c:if test="${QX.edit == 1 }">
																		<a class="btn btn-xs btn-success" title="下载"
																			onclick="window.location.href='<%=basePath%>/fhfile/download.do?FHFILE_ID=${var.FHFILE_ID}'">
																			<i class="ace-icon fa fa-cloud-download bigger-120"
																			title="下载"></i>
																		</a>
																	</c:if>
																	<c:if test="${QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"
																			onclick="del('${var.FHFILE_ID}');"> <i
