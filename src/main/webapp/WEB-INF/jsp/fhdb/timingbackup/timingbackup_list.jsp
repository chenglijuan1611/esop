				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="timingbackup/list.do" method="post" name="Form"
								id="Form">
									...
									<tr>
										...
										<td style="vertical-align: top; padding-left: 2px;"><select
											...
											style="vertical-align: top; width: 120px;">
												...
												<option value="1"
													<c:if test="${pd.STATUS == 1 }">selected="selected"</c:if>>正在运行</option>
												<option value="2"
													<c:if test="${pd.STATUS == 2 }">selected="selected"</c:if>>已经停止</option>
										</select></td>
										<c:if test="${QX.cha == 1 }">
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center'><label class="pos-rel"><input
																	...
																	value="${var.TIMINGBACKUP_ID}" class="ace" /><span
															...
															<td class="center"><c:if
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<c:if test="${QX.edit == 1 }">
																		<a id="offing1${vs.index+1}"
																			<c:if test="${var.STATUS == '1'}">style="display: none;"</c:if>
																			class="btn btn-info btn-xs" title="启动"
																			onclick="onoff('${var.TIMINGBACKUP_ID}','1',this.id,'${vs.index+1}');">
																			<i class="ace-icon glyphicon glyphicon-play"
																			title="启动"></i>
																		</a>
																		<a id="oning1${vs.index+1}"
																			<c:if test="${var.STATUS == '2'}">style="display: none;"</c:if>
																			class="btn btn-info btn-xs" title="关闭"
																			onclick="onoff('${var.TIMINGBACKUP_ID}','2',this.id,'${vs.index+1}');">
																			<i class="ace-icon glyphicon glyphicon-off"
																			title="关闭"></i>
																		</a>
																		<a class="btn btn-xs btn-success" title="编辑"
																			onclick="edit('${var.TIMINGBACKUP_ID}');"> <i
																	...
																	<c:if test="${QX.del == 1 }">
																		<a class="btn btn-xs btn-danger"
																			onclick="del('${var.TIMINGBACKUP_ID}');"> <i
																...
																<div class="hidden-md hidden-lg">
																	<div class="inline pos-rel">
																		...
																		<ul
																			...
																			<c:if test="${QX.edit == 1 }">
																				<li><a id="offing2${vs.index+1}"
																					<c:if test="${var.STATUS == '1'}">style="display: none;cursor:pointer;"</c:if>
																					onclick="onoff('${var.TIMINGBACKUP_ID}','1',this.id,'${vs.index+1}');"
																					class="tooltip-info" data-rel="tooltip" title="启动">
																						<span class="blue"> <i
																							class="ace-icon glyphicon glyphicon-play"
																							title="启动"></i>
																					</span>
																				</a></li>
																				<li><a id="oning2${vs.index+1}"
																					<c:if test="${var.STATUS == '2'}">style="display: none;cursor:pointer;"</c:if>
																					onclick="onoff('${var.TIMINGBACKUP_ID}','2',this.id,'${vs.index+1}');"
																					class="tooltip-info" data-rel="tooltip" title="关闭">
																						<span class="blue"> <i
																							class="ace-icon glyphicon glyphicon-off"
																							title="关闭"></i>
																					</span>
																				</a></li>
																				<li><a style="cursor: pointer;"
																					onclick="edit('${var.TIMINGBACKUP_ID}');"
																			...
																			<c:if test="${QX.del == 1 }">
																				<li><a style="cursor: pointer;"
																					onclick="del('${var.TIMINGBACKUP_ID}');"
