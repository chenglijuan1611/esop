				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<form action="loginimg/${msg }.do" name="Form" id="Form"
								method="post">
								<input type="hidden" name="LOGINIMG_ID" id="LOGINIMG_ID"
									value="${pd.LOGINIMG_ID}" /> <input type="hidden" value="no"
									id="hasTp1" />
									...
									<table id="table_report"
										...
										<tr>
											...
											<td><input type="text" name="NAME" id="NAME"
												value="${pd.NAME}" maxlength="30" placeholder="这里输入文件名"
												title="文件名" style="width: 98%;" /></td>
										</tr>
										<c:if test="${msg == 'save' }">
											<tr>
												<td
													style="width: 75px; text-align: right; padding-top: 13px;">图片:</td>
												<td id="FILEPATHn"><input type="file" name="File_name"
													id="uploadify1" keepDefaultStyle="true" /> <input
													type="hidden" name="FILEPATH" id="FILEPATH" value="" /></td>
											</tr>
										</c:if>
										<tr>
											<td
												style="width: 75px; text-align: right; padding-top: 13px;">排序:</td>
											<td><input type="number" name="FORDER" id="FORDER"
												value="${pd.FORDER}" maxlength="32" placeholder="这里输入排序"
												title="排序" style="width: 35%;" /></td>
										</tr>
										<tr>
											<td
												style="width: 75px; text-align: right; padding-top: 13px;">状态:</td>
											<td><select name="TYPE" id="TYPE" style="width: 35%;">
													<option value="1"
														<c:if test="${pd.TYPE == '1'}">selected</c:if>>启用</option>
													<option value="2"
														<c:if test="${pd.TYPE == '2'}">selected</c:if>>停用</option>
											</select></td>
										</tr>
										<tr>
