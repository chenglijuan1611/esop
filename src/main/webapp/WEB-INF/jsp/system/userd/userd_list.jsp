...
<body class="no-skin">
				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="userd/list.do" method="post" name="Form" id="Form">
								...
								<table id="simple-table"
									...
									<thead>
										<tr>
											...
											<th class="center">副职角色id</th>
											<th class="center">最后登录时间</th>
											<th class="center">用户登录ip地址</th>
													...
													<c:forEach items="${varList}" var="var" varStatus="vs">
														<tr>
															<td class='center'><label class="pos-rel"><input
																	type='checkbox' name='ids' value="${var.USERD_ID}"
															...
															<td class='center'>${var.RIGHTS}</td>
															<td class='center'>${var.ROLE_ID}</td>
															<td class='center'>${var.ROLE_IDS}</td>
															<td class='center'>${var.LAST_LOGIN}</td>
															<td class='center'>${var.IP}</td>
															<td class='center'>${var.STATUS}</td>
															<td class='center'>${var.EMAIL}</td>
															<td class='center'>${var.PHONE}</td>
															<td class='center'>${var.BZ}</td>
															<td class='center'>${var.DEPARTMENT_ID}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>userd/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>userd/delete.do?USERD_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>userd/goEdit.do?USERD_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>userd/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>userd/excel.do';
		}
	</script>
