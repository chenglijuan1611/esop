							...
							<form action="user/listUsers.do" method="post" name="userForm"
								...
								<table style="margin-top: 5px;">
									<tr>
										...
										<td><input name="ZDEPARTMENT_ID" id="ZDEPARTMENT_ID"
											type="hidden" value="${pd.ZDEPARTMENT_ID }" /> <input
											name="DEPARTMENT_ID" id="DEPARTMENT_ID" type="hidden"
											value="${pd.DEPARTMENT_ID }" /></td>
										<c:if test="${QX.cha == 1 }">
																...
																<div class="hidden-sm hidden-xs btn-group">
																	<c:if test="${QX.FHSMS == 1 }">
																		<a class="btn btn-xs btn-info" title='发送站内信'
																			onclick="sendFhsms('${user.USERNAME }');"> <i
																		...
																		<ul
																			...
																			<c:if test="${QX.FHSMS == 1 }">
																				<li><a style="cursor: pointer;"
																					onclick="sendFhsms('${user.USERNAME }');"
																			...
																			<c:if test="${QX.sms == 1 }">
																				<li><a style="cursor: pointer;"
																					...
																					title="发送短信"> <span class="blue"> <i
																							class="ace-icon fa fa-envelope-o bigger-120"></i>
																					</span>
																				</a></li>
																			</c:if>
																			<c:if test="${QX.edit == 1 }">
									...
									<table style="width: 100%;">
										<tr>
											<td style="vertical-align: top;"><c:if
												...
												</c:if> <c:if test="${QX.FHSMS == 1 }">
													<a title="批量发送站内信" class="btn btn-mini btn-info"
														onclick="makeAll('确定要给选中的用户发送站内信吗?');"><i
														class="ace-icon fa fa-envelope-o bigger-120"></i></a>
												</c:if> <c:if test="${QX.email == 1 }">
													<a title="批量发送电子邮件" class="btn btn-mini btn-primary"
														onclick="makeAll('确定要给选中的用户发送邮件吗?');"><i
														class="ace-icon fa fa-envelope bigger-120"></i></a>
												</c:if> <c:if test="${QX.sms == 1 }">
...
function toExcel(){
	...
	//alert($("#DEPARTMENT_ID").val($(this).attr("relValue"));
	window.location.href='<%=basePath%>user/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID+'&DEPARTMENT_ID='+DEPARTMENT_ID;
}
